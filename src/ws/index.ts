  ...
  constructor(server: Server, db: HubDB, webhookManager: WebhookManager, config: HubConfig) {
    ...
    this.heartbeatInterval = setInterval(() => {
      ...
      for (const client of [...this.clients]) {
        ...
        sendToClient(client, { type: 'server_ping', ts: now });
      }
    }, 30_000);
    ...
    this.staleSweepInterval = setInterval(() => {
      const now = Date.now();
      for (const client of [...this.clients]) {
        if (!this.clients.has(client)) continue; // already removed by close handler
        if (now - client.lastActivity > this.staleClientTimeout) {
          wsLogger.info({ orgId: client.orgId, botId: client.botId, idle_ms: now - client.lastActivity, connected_ms: now - client.connectedAt }, 'terminating stale client');
          client.ws.terminate();
        }
      }
    }, 60_000);
