...
export class HubWS implements WsHub {
  ...
  private staleSwapInterval: ReturnType<typeof setInterval> | null = null;
  private sessionHeartbeatInterval: ReturnType<typeof setInterval> | null = null;
  private sessionStore: SessionStore | null = null;
  private startedAt = Date.now();

  /** How long a client can be idle (no app-level messages) before being terminated. */
  private readonly staleClientTimeout = 120_000; // 2 minutes
  ...
  constructor(server: Server, db: HubDB, webhookManager: WebhookManager, config: HubConfig) {
    ...
    this.heartbeatInterval = setInterval(() => {
      ...
      for (const client of [...this.clients]) {
        ...
        client.alive = false;
        client.ws.ping();
        // W3: Send application-level ping that only real clients can answer
        // (protocol-level pong can be generated by reverse proxies, but app-level messages cannot)
        sendToClient(client, { type: 'server_ping', ts: now } as any);
      }
    }, 30_000);

    // W3: Stale-client sweep — terminate clients with no app-level activity
    this.staleSwapInterval = setInterval(() => {
      const now = Date.now();
      for (const client of [...this.clients]) {
        if (now - client.lastActivity > this.staleClientTimeout) {
          wsLogger.info({ orgId: client.orgId, botId: client.botId, idle_ms: now - client.lastActivity }, 'terminating stale client');
          client.ws.terminate();
        }
      }
    }, 60_000);
  ...
  private setupHandlers(client: WsClient) {
    client.ws.on('message', async (raw) => {
      // W3: Update lastActivity on every incoming application-level message
      ...
      try {
        ...
        // W3: Application-level pong response — no further processing needed
  ...
  getHealthStats(): { uptime_ms: number; connected_clients: number; connected_bots: number; stale_clients: number; ws_metrics: import('./metrics.js').WsMetrics } {
    const now = Date.now();
    const botIds = new Set<string>();
    let staleCount = 0;
    for (const client of this.clients) {
      if (client.botId) botIds.add(client.botId);
      if (now - client.lastActivity > this.staleClientTimeout) staleCount++;
    }
    return {
      uptime_ms: Date.now() - this.startedAt,
      connected_clients: this.clients.size,
      connected_bots: botIds.size,
      stale_clients: staleCount,
  ...
  async shutdown(code: number = 1001): Promise<void> {
    // Stop heartbeats and sweeps
    if (this.heartbeatInterval) {
      clearInterval(this.heartbeatInterval);
      this.heartbeatInterval = null;
    }
    if (this.staleSwapInterval) {
      clearInterval(this.staleSwapInterval);
      this.staleSwapInterval = null;
    }
