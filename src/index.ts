...
async function main() {
  ...
  // Ensure files directory exists (hierarchical: files/<org>/<shard>/, staging: files/_tmp/)
  ...
  // Clean up orphaned temp files from previous crashes (older than 1 hour)
  try {
  ...
  setInterval(() => {
    ...
    // Sweep orphaned temp upload files (older than 1 hour)
    try {
      const ONE_HOUR = 60 * 60 * 1000;
      for (const entry of fs.readdirSync(filesTmpDir)) {
        const fp = path.join(filesTmpDir, entry);
