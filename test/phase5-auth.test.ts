...
describe('Login-to-Register Auth Flow', () => {
  let env: TestEnv;
  let orgId: string;
  let orgSecret: string; // admin_secret is the org_secret for login
  ...
  it('rejects register when ticket belongs to different org', async () => {
    ...
    const { body: loginBody } = await api(env.baseUrl, 'POST', '/api/auth/login', {
      body: { org_id: org2.id, org_secret: org2.admin_secret },
    });
...
describe('Paginated Agents (GET /api/agents)', () => {
  ...
  beforeAll(async () => {
    ...
    // Register 5 agents via legacy API
    for (let i = 1; i <= 5; i++) {
      await env.registerAgent(orgApiKey, `agent-${String(i).padStart(2, '0')}`);
    }
  });
...
describe('Paginated Org Threads (GET /api/org/threads)', () => {
  ...
  beforeAll(async () => {
    ...
    // Register an agent
    const { token } = await env.registerAgent(orgApiKey, 'thread-maker');
    agentToken = token;
  ...
  it('returns paginated response with limit param', async () => {
    const { status, body } = await api(env.baseUrl, 'GET', '/api/org/threads?limit=3', {
      token: orgApiKey,
      headers: { 'X-Admin-Secret': orgSecret },
    });
    expect(status).toBe(200);
    expect(body.items).toBeDefined();
...
describe('Paginated Artifacts (GET /api/org/threads/:id/artifacts)', () => {
  ...
  beforeAll(async () => {
    ...
    // Register agent
    const { token } = await env.registerAgent(orgApiKey, 'artifact-maker');
    agentToken = token;
...
describe('Paginated Thread Messages (GET /api/org/threads/:id/messages)', () => {
  ...
  beforeAll(async () => {
    ...
    const { token } = await env.registerAgent(orgApiKey, 'msg-maker');
    agentToken = token;
  ...
  it('returns cursor-based response with limit param', async () => {
  ...
  it('paginates with before=message_id (cursor)', async () => {
    ...
    const { body: page1 } = await api(env.baseUrl, 'GET', `/api/org/threads/${threadId}/messages?limit=3`, {
      token: orgApiKey,
      headers: { 'X-Admin-Secret': orgSecret },
    });
  ...
  it('returns all 6 messages across pages with no duplicates', async () => {
    ...
    for (let page = 0; page < 10; page++) {
      const url = before
        ? `/api/org/threads/${threadId}/messages?limit=2&before=${before}`
        : `/api/org/threads/${threadId}/messages?limit=2`;
      const { body } = await api(env.baseUrl, 'GET', url, {
        token: orgApiKey,
        headers: { 'X-Admin-Secret': orgSecret },
      });
...
describe('Org Lifecycle and Auth', () => {
  ...
  beforeAll(async () => {
    ...
    orgId = orgBody.id;
    orgSecret = orgBody.admin_secret;
  });
...
describe('Super Admin Org Management', () => {
  ...
  it('lists orgs via super admin (without exposing keys)', async () => {
    ...
    for (const org of body) {
      expect(org.api_key).toBeUndefined();
      expect(org.admin_secret).toBeUndefined();
    }
  });
  ...
  it('full lifecycle: create org, login, register, use API', async () => {
    ...
    const orgSecret = orgBody.admin_secret;
...
describe('Mixed Auth Types', () => {
  ...
  beforeAll(async () => {
    ...
    // Register agent via legacy flow
    const { token } = await env.registerAgent(orgApiKey, 'mixed-agent');
    agentToken = token;
  });

  afterAll(() => env.cleanup());

  it('org API key works as Bearer for org-level endpoints', async () => {
...
describe('WS Ticket Exchange (POST /api/ws-ticket)', () => {
  ...
  beforeAll(async () => {
    ...
    const { token } = await env.registerAgent(orgApiKey, 'ws-agent');
    agentToken = token;
  });
  ...
  it('org key can exchange for ws-ticket', async () => {
    const { status, body } = await api(env.baseUrl, 'POST', '/api/ws-ticket', {
      token: orgApiKey,
    });
    expect(status).toBe(200);
